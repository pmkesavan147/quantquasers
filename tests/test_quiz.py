...
FULL = {
    ...
    "open_outlook": "Bullish on banks, cautious on IT",
    "open_goal": "Beat my FD returns",
}
...
def test_a_confident_cached_read_can_move_the_band_one_notch():
    ...
    answers = {
        ...
        "uses_leverage": False,
        "open_outlook": "I check charts every hour and hate holding overnight",
        "open_goal": "trade actively",
    }
    profile_before, _ = to_profile(answers, use_gemma=False)
    band_before = risk_band(profile_before)

    prompt = PROFILE_PROMPT.format(
        rubric_score=__import__("trading.allocation", fromlist=["x"])
        .rubric_score(profile_before),
        ...
        outlook=answers["open_outlook"],
        goal=answers["open_goal"],
    )
    client.cache_put(
